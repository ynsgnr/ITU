...
%apply PCA
    ...
    for row=1:F
        for col=1:row
            cov=0;
            for t=1:T
                cov=cov+((data(t,row)-means(row))*(data(t,col)-means(col)));
            end
            covMat(row,col)=cov/(T-1);
            covMat(col,row)=covMat(row,col);
        end
    end
    [V,D]=eig(covMat);
    D=max(D); %all eigenvalues
    max1=0;
    maximum=0;
    for f=1:F
        if D(f)>maximum
    ...
    PCAred=[V(:,max1),V(:,max2)];
    data=data(:,1:F)*PCAred;
    tra1=[data,tra1(:,F+1)];
    tmp=test;
    tmp=tmp(:,1:F)*PCAred;
    test=[tmp,test(:,F+1)];
